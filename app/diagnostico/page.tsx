...
function routeRecommendation(input: {
  ...
  if (secondary && secondary !== primary) {
    if (secondary === "moduz") why.push("Alternativa: Moduz+ pode ser o próximo passo quando a operação exigir escala.");
...
export default function DiagnosticoPage() {
  ...
  const summaryText = useMemo(() => {
    const s = symptoms.map((k) => SYMPTOMS.find((x) => x.key === k)?.title).filter(Boolean);
